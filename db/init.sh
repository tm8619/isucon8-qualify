...
mysql -uisucon torb -e 'INSERT INTO cancelled_reservations SELECT * FROM reservations where canceled_at IS NOT NULL;SET SQL_SAFE_UPDATES = 0;DELETE FROM `reservations` where `canceled_at` IS NOT NULL;ALTER TABLE `reservations` DROP COLUMN `canceled_at`;'
